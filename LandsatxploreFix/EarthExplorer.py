# module from https://github.com/yannforget/landsatxplore/blob/master/landsatxplore/earthexplorer.py
# corrected from https://github.com/yannforget/landsatxplore/issues/82
...
from LandsatxploreFix.util import guess_dataset, is_display_id
...
### GDL has augmented this list with 2 additional hash codes that are used for Landsat 8 and 9
### (landsat_ot_c2_l2) which are now tried when downloading is attempted
DATA_PRODUCTS = {
    ...
    def download(self, identifier, output_dir, dataset=None, timeout=300, skip=False):
        ...
        try:
            ...
            try:
                ...
                url = EE_DOWNLOAD_URL.format(
                    data_product_id=DATA_PRODUCTS_III[dataset], entity_id=entity_id
                )
                filename = self._download(url, output_dir, timeout=timeout, skip=skip)
        return filename